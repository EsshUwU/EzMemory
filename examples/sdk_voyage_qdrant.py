"""
SDK example: VoyageAI embeddings + Qdrant vector store (no EzMemory config file).

Requirements:
    pip install ezmemory voyageai qdrant-client
...
def main() -> None:
    ...
    if not api_key:
        raise SystemExit("Please set VOYAGE_API_KEY in your environment.")
    ...
    embedding = VoyageEmbedding(api_key=api_key, model="voyage-4")
    ...
    collection_name = "ezmemory_sdk_voyage_qdrant"
    ...
    # 4) Add memories
    examples = [
        "User is learning about large language models.",
        "User works as a machine learning engineer.",
        "User enjoys reading research papers on weekends.",
    ]
